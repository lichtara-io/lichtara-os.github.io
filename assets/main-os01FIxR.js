      ...
      <dl class="grid gap-y-3 text-sm">
        ${[{label:"Status",value:t.status||"Unknown"},{label:"Role",value:t.role||"-"},{label:"Maturity",value:t.maturity||"-"},{label:"Public",value:t.public?"Sim":"Não"},{label:"Description",value:t.description||"-"}].map(n=>`
          <div class="grid grid-cols-[140px,1fr] gap-x-4">
            <dt class="text-gray-600 font-medium">${n.label}:</dt>
            <dd class="text-gray-900">${n.value}</dd>
          </div>
        `).join("")}
      </dl>
    </div>
...
${t.prompt}
  ...
  `,e.appendChild(a);const s=document.createElement("div");s.id="pipeline-status",s.className="hidden p-4 bg-slate-900/50 rounded border border-slate-800",e.appendChild(s);const r=document.createElement("div");r.id="pipeline-results",r.className="space-y-4",e.appendChild(r);const l=a.querySelector("#run-pipeline"),d=a.querySelector("#agent-select"),f=a.querySelector("#input-data");l.addEventListener("click",()=>{const i=d.value,c=f.value.trim();if(!i){g("Selecione um agente");return}if(!c){g("Digite um input de exemplo");return}b(i,c)});function g(i){s.className="p-4 bg-red-900/30 border border-red-600/30 rounded",s.innerHTML=`<p class="text-red-300 text-sm">${i}</p>`,setTimeout(()=>s.classList.add("hidden"),3e3)}function b(i,c){const u=t.agents.find(o=>o.code===i);u&&(s.classList.remove("hidden"),s.className="p-4 bg-blue-900/30 border border-blue-600/30 rounded",s.innerHTML='<p class="text-blue-300 text-sm">Executando pipeline...</p>',setTimeout(()=>{const o=t.pipelineSimulator.simulate(u,c);y(o),s.classList.add("hidden")},1500))}function y(i){const c=document.createElement("div");for(c.className="bg-slate-900/60 border border-slate-800 rounded p-4 space-y-3",c.innerHTML=`
  ...
  `,e.appendChild(a);const s=document.createElement("div");s.className="flex flex-wrap gap-2",s.innerHTML=`
    ...
    `).join("")}
  `,e.appendChild(s);const r=document.createElement("div");r.id="telemetry-logs",r.className="bg-slate-950 border border-slate-800 rounded h-96 overflow-y-auto font-mono text-xs",e.appendChild(r);let l="all",d=!0;const f=n.querySelector("#clear-logs"),g=n.querySelector("#toggle-auto-scroll"),b=s.querySelectorAll(".filter-btn");f.addEventListener("click",()=>{t.telemetry.clearEvents(),c()}),g.addEventListener("click",()=>{d=!d,g.textContent=`Auto-scroll: ${d?"ON":"OFF"}`,g.dataset.auto=d.toString()}),b.forEach(o=>{o.addEventListener("click",()=>{b.forEach(p=>{p.className=p.className.replace(/bg-emerald-\w+ border-emerald-\w+ text-white/,"bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700")}),o.className=o.className.replace(/bg-slate-\w+ border-slate-\w+ text-slate-\w+ hover:bg-slate-\w+/,"bg-emerald-600 border-emerald-600 text-white"),l=o.dataset.level,c()})});const y=setInterval(c,1e3),i=new MutationObserver(o=>{o.forEach(p=>{p.type==="childList"&&p.removedNodes.forEach(v=>{v===e&&(clearInterval(y),i.disconnect())})})});i.observe(document.body,{childList:!0,subtree:!0});function c(){const o=t.telemetry.getEvents(),p=l==="all"?o:o.filter(m=>m.level===l),v=a.querySelector("#total-events"),w=a.querySelector("#last-event"),S=a.querySelector("#active-agents"),L=a.querySelector("#event-rate");v.textContent=o.length,w.textContent=o.length>0?new Date(o[o.length-1].timestamp).toLocaleTimeString():"-",S.textContent=t.agents.filter(m=>m.status==="active").length;const $=Date.now()-6e4,C=o.filter(m=>m.timestamp>$);L.textContent=`${C.length}/min`,r.innerHTML=p.map(m=>u(m)).join(""),d&&(r.scrollTop=r.scrollHeight)}function u(o){const p={info:"text-blue-400",warning:"text-yellow-400",error:"text-red-400",debug:"text-slate-500"},v=new Date(o.timestamp).toLocaleTimeString(),w=p[o.level]||"text-slate-300";return`
