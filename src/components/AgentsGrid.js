...
export function AgentsGrid() {
  return `
    <div id="agents-grid-root" class="agents-grid-container">
...
export async function initializeAgentsGrid() {
  try {
    ...
    renderAgentsGrid({ agents: agentsData });
    TelemetryService.track('agents_grid.initialized', { total_agents: agentsData.length });
  } catch (error) {
    console.error('Erro ao carregar agentes:', error);
    const container = document.getElementById('agents-grid-root');
    if (container) {
      container.innerHTML = `
...
function renderAgentsGrid(state) {
  const container = document.getElementById('agents-grid-root');
  if (!container) return;
  ...
  // Painel de detalhes inline (não modal)
  const detailsWrap = document.createElement('div');
  detailsWrap.id = 'agent-details';
  detailsWrap.className = 'ag-details-wrap';
  ...
  function openDetails(agent) {
    detailsWrap.innerHTML = `
      ...
      ">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem;">
          <div style="background: rgba(248, 250, 252, 0.6); padding: 1rem; border-radius: 8px; border: 1px solid var(--border-color);">
    ...
    TelemetryService.track('agent.selected', { agent_id: agent.id });
  }
}
